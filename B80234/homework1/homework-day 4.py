...
def calculate_word(sorted_dict,total_count): #计算累积词频
    ...
    sortd_cal_list = sorted(cal_list.iteritems(), key=lambda d:d[1], reverse=False)
...
# 4.输出成csv
def print_to_csv(Flis, to_file_path ):
...
def main():
    ...
    # 4. 输出文件
    print_to_csv(extract_dict, 'output/test-yang.csv')
