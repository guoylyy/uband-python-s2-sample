...
def word_recite(word_table,word_amout,day):
    recite_list = []
    daily_word = []
    day = 1
    for word in word_table:
        if word[1] != 'no meaning':
            recite_list.append(word)
    for day in range(1,len(word_table)/word_amout):
...
def main():
    ...
    daily_word = word_recite(word_table,50,3)

    # 4. 输出文件
    print_to_csv(daily_word, 'output/test-yang.csv')
